...
hw_info_t hw_info() { return hw_info_; }

void hw_init(uint8_t raw) { hw_info_.value = raw; }
...
static void sos_impl(uint8_t no, uint8_t delay)
{
    while (0 != no)
    {
        dbg_led_on();
...
void tlog_dump(void)
{
    ...
    usart0_send_str("\n<<<\n");
    snprintf(
        buf, sizeof(buf), "TLOG %0" PRIu16 "\n", tlog_end() - tlog_begin());
    usart0_send_str(buf);
    usart0_send_str_r(tlog_begin(), tlog_end());
    usart0_send_str("\n>>>\n");
}

uint16_t tlog_timestamp(void) { return TMR1_RD16_CNTR(); }
