...
static tx_ctrl_t tx0_;
#ifdef USART_DBG_CNTRS
static usart_cntrs_t tx0_cntrs_;
...
static rx_ctrl_t rx0_;
static usart_rxflags_t rx0_flags_;
#ifdef USART_DBG_CNTRS
static usart_cntrs_t rx0_cntrs_;
...
const char *usart0_send_str_r(const char *begin, const char *const end)
{
    ...
    while (NULL == end && *begin != '\0' || NULL != end && begin != end)
    {
...
{
    if (NULL == begin || NULL == end || begin == end) return;
    ...
    uintptr_t user_data)
{
    ...
    if (NULL == tx0_.begin && NULL == tx0_.end && NULL == tx0_.complete_cb
        && 0 == tx0_.user_data)
    {
        tx0_.begin       = begin;
        tx0_.end         = end;
    ...
    uintptr_t user_data)
{
    ...
    if (NULL == rx0_.begin && NULL == rx0_.end && NULL == rx0_.next
        && NULL == rx0_.pred_cb && NULL == rx0_.complete_cb
        && 0 == rx0_.user_data)
    {
        rx0_.begin       = begin;
        rx0_.end         = end;
...
{
    if (NULL == rx0_.complete_cb
        /* there is no data in buffer */
        || rx0_.begin == rx0_.next)
        return;
...
void usart0_async_recv_cb(usart_rx_recv_cb_t recv_cb, uintptr_t user_data)
{
    ...
    if (NULL == rx0_.begin && NULL == rx0_.end && NULL == rx0_.next
        && NULL == rx0_.pred_cb && NULL == rx0_.complete_cb
        && NULL == rx0_.recv_cb && 0 == rx0_.user_data)
    {
...
ISR(USART_UDRE_vect)
{
    ...
    while (tx0_.begin != tx0_.end && USART0_TX_READY())
    {
...
ISR(USART_RX_vect)
{
    ...
    while (rx0_.next != rx0_.end && USART0_RX_READY())
    {
        rx0_flags_.fop_errors = USART0_FOP_ERRORS();
    ...
    if (rx0_flags_.full || rx0_flags_.aborted)
    {
        USART0_RX_INT_DISABLE();
        rx_complete();
    }
